    ...
    return (
        <header className='p-[15px] fixed w-full box-border bg-[#f4f0ea] sm:p-[20px]'>
            <div className="bar flex items-center justify-center text-[12px] uppercase relative sm:text-[15px]">
                <Link href='/' className='absolute left-0'>Yohan Baes</Link>
                <div onMouseDown={() => {setIsActive(!isActive)}} className="el flex items-center gap-[8px] cursor-pointer">
                    <div className={`${styles.burger} ${isActive ? styles.burgerActive : ""}`}></div>
                    <div className={styles.label}>
                        <motion.p variants={opacity} animate={isActive ? "closed" : "open"} className='m-0'>Menu</motion.p>
                        <motion.p variants={opacity} animate={!isActive ? "closed" : "open"} className='m-0'>Close</motion.p>
                    </div>
                </div>
                <motion.div variants={opacity} animate={isActive ? "closed" : "open"} className="shopContainer absolute right-0 flex items-center gap-[30px]">
                    <p className="shop m-0 hidden sm:block">Shop</p>
                    <div className="el flex items-center gap-[8px] cursor-pointer">
